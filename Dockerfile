...
RUN mkdir C:\tools `
    && mkdir C:\tools\java `
    && curl -L --output C:\tools\java\jdk.zip https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.18%2B10/OpenJDK11U-jdk_x64_windows_hotspot_11.0.18_10.zip `
    && tar xzf C:\tools\java\jdk.zip -C C:\tools\java `
    && del C:\tools\java\jdk.zip `
    && mkdir C:\tools\maven `
    && curl -L --output C:\tools\maven\maven.zip https://dlcdn.apache.org/maven/maven-3/3.8.7/binaries/apache-maven-3.8.7-bin.zip `
    && tar xzf C:\tools\maven\maven.zip -C C:\tools\maven `
    && del C:\tools\maven\maven.zip
ENV JAVA_HOME=C:\tools\java\jdk-11.0.18+10
ENV MAVEN_HOME=C:\tools\maven\apache-maven-3.8.7
